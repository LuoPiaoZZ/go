...
// author by : nizonglong
// Register - only register srv's job model, start auto.
func (c *CronSchduler) RegisterAutoStart(srv string, model *JobModel) error {
	return c.reset(srv, model, true, true)
}
...
// author by : nizonglong
// StopService - stop job by serviceName
func (c *CronSchduler) StopServiceWithoutDelete(srv string) error {
	c.Lock()
	defer c.Unlock()

	job, ok := c.tasks[srv]
	if !ok {
		return errors.New("SRV DOESN'T EXIST")
	}
...
// author by : nizonglong
// start one job
func (c *CronSchduler) StartJob(job *JobModel) {
	job.running = true
	job.exited = false
	job.runLoop(c.wg)
	// 由于关闭是使用 kill 关闭了 notifyChan，因此在启动的时候需要重新开启，避免阻塞
	job.notifyChan = make(chan int, 1)
}
...
// author by : nizonglong
func (j *JobModel) GetSrv() string {
	return j.srv
}
func (j *JobModel) GetSpec() string {
	return j.spec
}
func (j *JobModel) GetRunning() bool {
	return j.running
}
func (j *JobModel) GetExited() bool {
	return j.exited
}
